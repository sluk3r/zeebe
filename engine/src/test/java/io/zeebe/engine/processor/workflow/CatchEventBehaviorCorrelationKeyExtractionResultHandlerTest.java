...
public class CatchEventBehaviorCorrelationKeyExtractionResultHandlerTest {
  ...
  @Test
  public void shouldThrowExceptionForDoubleNumberResult() {
    // given
    final EvaluationResult mockEvaluationResult = mock(EvaluationResult.class);
    when(mockEvaluationResult.getExpression()).thenReturn(TEST_EXPRESSION);
    when(mockEvaluationResult.getType()).thenReturn(ResultType.NUMBER);

    when(mockEvaluationResult.getNumber()).thenReturn(Double.valueOf(42.5));

    // when + then
    assertThatThrownBy(() -> sutResultHandler.handleEvaluationResult(mockEvaluationResult))
        .isExactlyInstanceOf(MessageCorrelationKeyException.class)
        .hasMessage(
            "Failed to extract the correlation key for 'expression'. Value was not an integer: 42.500000");
  }

  @Test
  public void shouldThrowExceptionForFloatNumberResult() {
    // given
    final EvaluationResult mockEvaluationResult = mock(EvaluationResult.class);
    when(mockEvaluationResult.getExpression()).thenReturn(TEST_EXPRESSION);
    when(mockEvaluationResult.getType()).thenReturn(ResultType.NUMBER);
    when(mockEvaluationResult.getNumber()).thenReturn(Float.valueOf(123.4f));

    // when + then
    assertThatThrownBy(() -> sutResultHandler.handleEvaluationResult(mockEvaluationResult))
        .isExactlyInstanceOf(MessageCorrelationKeyException.class)
        .hasMessage(
            "Failed to extract the correlation key for 'expression'. Value was not an integer: 123.400002");
  }
  ...
  public void shouldThrowExceptionForArrayResult() {
    ...
    assertThatThrownBy(() -> sutResultHandler.handleEvaluationResult(mockEvaluationResult))
        .isExactlyInstanceOf(MessageCorrelationKeyException.class)
        .hasMessage(
            "Failed to extract the correlation key for 'expression': Array of multiple values found.");
  }
}
