...
public final class WorkflowPersistenceCache {
  ...
  private final LatestWorkflowVersion latestWorkflowVersion = new LatestWorkflowVersion();
  ...
  private void updateLatestVersion(final Workflow workflow) {
    ...
    if (workflow.getVersion() > latestVersion) {
      latestWorkflowVersion.set(workflow.getVersion());
  ...
  public DirectBuffer getLatestVersionDigest(final DirectBuffer processId) {
    workflowId.wrapBuffer(processId);
    final Digest dbBuffer = digestByIdColumnFamily.get(workflowId);
    return dbBuffer == null || digest.get().byteArray() == null ? null : dbBuffer.get();
  }
}
